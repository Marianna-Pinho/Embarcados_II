...
//--Marianna
static int age_Conjunto(int gpio, int tam, int value){
	int ret = 0,i = 0;
	for(i=0;i<=tam;i++){
			ret = gpio_request(gpio, "cmd_gpio");			//ele requisita esse pino como gpio
			if (ret && ret != -EBUSY) {							//verifica se deu certo
			printf("gpio: requesting pin %u failed\n", gpio);
			return -1;
			}
			printf("gpio: pin %d value is %d\n", gpio,value);	//mostra na tela o pino e seu valor
			gpio_direction_output(gpio, value);					//seta o pino
			
			if (ret != -EBUSY){									//Se o pino não estiver mais sendo usado, libera ele
				gpio_free(gpio);
			}
			gpio++;												//Incrementa para o próximo pino		
	}
	return 0;
}
static int do_gpio(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	unsigned int gpio, gpio2; //--Marianna
	enum gpio_cmd sub_cmd;
	int value;
	const char *str_cmd, *str_gpio = NULL, *str_gpio2 = NULL;
	int ret;
 ...
 show_usage:
...
#elif defined(CONFIG_DM_GPIO)
		return cmd_process_error(cmdtp, do_gpio_status(all, str_gpio2));	//--Marianna*/
#else
		goto show_usage;
#endif
	}
	...
	switch (*str_cmd) {
		...
		case 'b': sub_cmd = GPIO_BLINK;	 break;
		case 'v': sub_cmd = GPIO_SET_CONJUNTO;	break;
		case 'l': sub_cmd = GPIO_CLEAR_CONJUNTO; break;	
	...
	/*ret = gpio_lookup_name(str_gpio2, NULL, NULL, &gpio2); //--Marianna
	if (ret) {
		printf("GPIO: '%s' not found\n", str_gpio2);
		return cmd_process_error(cmdtp, ret);
	}*/
	...
	if (sub_cmd == GPIO_INPUT) {
		...
		case GPIO_BLINK:
			...
			break;
		case GPIO_SET_CONJUNTO:
			/* turn the gpio name into a gpio number */
			if (!str_gpio2)		//--Marianna
				goto show_usage;
			...
			gpio2 = name_to_gpio(str_gpio2);	//transforma a string numero em um numero inteiro
			printf("GPIO2 %d\n",gpio2);
		
			if (gpio2 < 0)						//verifica se é um valor válido
			goto show_usage;

			value = 1;

			ret = age_Conjunto(gpio, gpio2-gpio,value);		//int age_Conjunto(int gpio, int tam, int value);	Função que acende ou apaga todos os leds
			if(ret == -1)
				return -1;
			break;
		case GPIO_CLEAR_CONJUNTO:
			/* turn the gpio name into a gpio number */
			if (!str_gpio2)		//--Marianna
				goto show_usage;
			...
			ret = age_Conjunto(gpio, gpio2-gpio,value);
			if(ret == -1)
				return -1;
			break;
		default:
